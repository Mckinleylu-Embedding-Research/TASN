...
class CentralityEncoding(nn.Module):
    ...
    def __init__(self, hidden_dim):
        ...
        self.feature_encoder = nn.Linear(1433, hidden_dim)  #x输入维度为7 输出维度为hidden_dim
        self.degree_encoder = nn.Embedding(2709, hidden_dim)
...
class EdgeEncoding(nn.Module):
    ...
    def __init__(self, head_num):
        ...
        self.edge_encoder = nn.Linear(2, head_num)
        ...
        for i in range(x.shape[0]):
            for j in range(x.shape[0]):
                ...
                for path in shortest_paths:
                    ...
                    for e in path:
                        e_attr = edge_attr[e]
                        tmp_my = torch.tensor((e_attr,0), device=x.device)
                        #e_tensor = self.edge_encoder(e_attr)
                        e_tensor = self.edge_encoder(tmp_my)
                        val += e_tensor
                    if len(path) > 0:
                        val /= len(path)
                    out[i, j] += val
